...
public interface MsgRepository extends JpaRepository<Msg, Long> {
    ...
    @Query("SELECT DISTINCT  m FROM Msg m WHERE m.ROOM_ID = :roomId AND m.STATUS = 'ACTIVE' ")
    List<Msg> findMsgsByRoomId(Long roomId);

    @Query("SELECT COUNT(m) FROM Msg m WHERE m.ROOM_ID = :roomId AND m.STATUS = :active AND m.EMAIL <> :ownerEmail")
    Long countUnreadMessages(@Param("roomId") Long roomId,
                             @Param("ownerEmail") String ownerEmail,
                             @Param("active") MsgStatus active);
    ...
    // 안읽은 메시지 개수 조회
    @Query("SELECT COUNT(m) FROM Msg m " +
            "WHERE m.ROOM_ID = :roomId " +
            "  AND m.isRead = false " +
            "  AND m.STATUS = 'ACTIVE' " +
            "  AND m.EMAIL <> :recipientEmail")
    int countUnreadMessagesByRoomAndRecipient(@Param("roomId") Long roomId, @Param("recipientEmail") String recipientEmail);
